...
with pdfplumber.open(pdf_path) as pdfreader:
    # Extract PDF title from first line of content
    first_line = pdfreader.pages[0].extract_text().split('\n')[0]
    pdf_title = first_line.strip() if first_line else "Unknown Title"
...
# Display the PDF title outside the condition block
st.write("PDF Title:", pdf_title)
